var p = [];
var quotes = [
{quote: 'As we grow old…the beauty steals inward.', source: 'Ralph Waldo Emerson', citation: '', year: ''},
{quote: "Don't cry because it's over, smile because it happened.", source: 'Dr. Seuss', citation: '', year: ''},
{quote: "If you want to know what a man's like, take a good look at how he treats his inferiors, not his equals.", source: 'J.K. Rowling', citation: 'Harry Potter and the Goblet of Fire', year: '2000'},
{quote: 'Without music, life would be a mistake.', source: 'Friedrich Nietzsche', citation: 'Twilight of the Idols' , year: ''},
{quote: 'Darkness cannot drive out darkness: only light can do that. Hate cannot drive out hate: only love can do that', source: 'Martin Luther King Jr', citation: '', year:'1986' }
];
...
if (p.length >= 5) {p = []}; // this is making sure that the array is reset once it hits 5 numbers
return quo[q];              // returns the value we got, i did not give use .quote at the end in order to make the function reusable for source, citation, year, and so on.
}
...
if (gq.citation !== '')
{ cit =  '<span class="citation">' + gq.citation + '</span>' } else cit = ''; //conditional to make sure that citation has something in it, if not, then it will not show
if (gq.year !== '')
{ yer =  '<span class="year">' + gq.year + '</span>' } else yer = '';    // same thing but with year
var q = '<p class="quote">' + gq.quote + '</p>';                       // spliting up the text
    q += '<p class="source">' + gq.source + cit + yer + '</p>';

document.getElementById('quote-box').innerHTML = q; // this will then make the quotes apear in the browser in the box itself.
}
function rgbColor() {
